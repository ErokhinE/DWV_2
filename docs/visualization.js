...
function init() {
    // Create simulated socket instead of connecting to a real one
    ...
    // Start the data simulation immediately
    startDataSimulation();
    
    // Simulate connection event
    simulatedSocket.emit('connect');
    
    // Simulate initial statistics
    simulatedSocket.emit('stats_update', {
        total_packets: 0,
        suspicious_packets: 0
    });
}

// Define some major cities with their coordinates for simulation
const cities = [
...
function startDataSimulation() {
    // Generate random traffic every 2-4 seconds
    setInterval(() => {
        generateRandomTraffic();
    }, Math.random() * 2000 + 2000);
}

// Generate a single random traffic connection
function generateRandomTraffic() {
    ...
    const trafficData = {
        ...
        protocol: suspicious ? 
            ["SSH", "TOR", "BitTorrent", "Unknown"][Math.floor(Math.random() * 4)] : 
            ["HTTP", "HTTPS", "FTP", "DNS", "SMTP"][Math.floor(Math.random() * 5)],
        size: Math.floor(Math.random() * 1000) + 100
    };
    
    // Emit the traffic data event
    simulatedSocket.emit('new_traffic', trafficData);
    
    // If suspicious, emit an alert
    if (suspicious) {
        simulatedSocket.emit('suspicious_alert', {
            message: `Suspicious ${trafficData.protocol} traffic detected from ${source.name} to ${destination.name}`,
            packet: trafficData
        });
    }
    
    // Update statistics every few packets
    if (stats.total_packets % 5 === 0) {
        simulatedSocket.emit('stats_update', stats);
    }
}
